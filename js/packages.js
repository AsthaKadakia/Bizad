...
function galleryFilters() {
    ...
    filters.hide();
    $('#filter-toggle').show();
    $('#filter-toggle').on('mousedown', function() {
        var trigger = $(this),
            altText = trigger.attr('data-alttext');
        trigger.attr('data-alttext', trigger.text());
    ...
    filters.on('change', 'select', function() {
        $('#gallery').load('?format=html', {
            'platformId' : $('#frm-filter-platform').val(),
            'objectiveId' : $('#frm-filter-objective').val(),
            'sectorId' : $('#frm-filter-business-sector').val(),
            'categoryId' : $('#frm-filter-type').val()
        });
    });
}
...
function buildGalleryUrl() {
    var querystring = '';
    $('select', '#gallery-filters').each(function(){
        var input = $(this);
...
function contactFilters() {
    var filters = $('#contact-filters');
    $('input', filters).hide();
    filters.on('change', 'select', function() {
        $('#contact-info').load('?format=html', {
            'regionId' : $('#frm-filter-region').val()
        });
    });
}
...
function specFilters() {
    ...
    $('input', filters).hide();
    filters.on('change', 'select', function() {
        $('#spec-listing').load('?format=html', {
            'regionId' : $('#frm-filter-region').val(),
            'platformId' : $('#frm-filter-platform').val(),
            'categoryId' : $('#frm-filter-category').val()
        });
    });
}

function codeFilters() {
    var filters = $('#code-filters');
    $('input', filters).hide();
    filters.on('change', 'select', function() {
        $('#code-listing').load('?format=html', {
            'platformId' : $('#frm-filter-platform').val()
        });
    });
}

function galleryLoad(target, start) {
    target.siblings('.action').addClass('loading');
    $.ajax({
        type: 'POST',
        url: '?format=html', 
        data: {
            ...
            'platformId' : $('#frm-filter-platform').val(),
            'objectiveId' : $('#frm-filter-objective').val(),
            'audienceId' : $('#frm-filter-audience').val(),
            'sectorId' : $('#frm-filter-business-sector').val(),
            'adTypeId' : $('#frm-filter-type').val()
        },
        success: function(data) {
            target.siblings('.action').removeClass('loading');
            target.append(data);
        }
    });
}

function specLoad(target, start) {
    target.siblings('.action').addClass('loading');
    $.ajax({
        type: 'POST',
        url: '?format=html', 
...
function newsLoad(target, start) {
    target.siblings('.action').addClass('loading');
    $.ajax({
        type: 'POST',
        url: '?format=html', 
...
function codeLoad(target, start) {
    target.siblings('.action').addClass('loading');
    $.ajax({
        type: 'POST',
        url: '?format=html', 
...
function saveContentToProfile(trigger, id, contentType) {
    trigger.addClass('loading').html('Saving&hellip;');
    $.ajax({
        type: 'POST',
        url: '/save-to-profile', 
        data: {
...
function removeContentFromProfile(trigger, id, contentType) {
    trigger.addClass('loading').html('Removing&hellip;');
    $.ajax({
        type: 'POST',
        url: '/remove-from-profile', 
        data: {
            'id' : id,
            'type' : contentType,
            'format' : 'html'
        ...
        success: function(data) {
            if (trigger.parents('.button').hasClass('nosave')) {
...
function buildNav() {
    var nav = $('#main-nav'),
        mobileNav = $('<select id="main-nav-mobile"/>'),
        subNav = $('#inpage-nav'),
        mobileSubNav = $('<select id="sub-nav-mobile"/>');

    $('a', nav).each(function() {
        var selected = '';
        var item = $(this);
        if (item.parents('li').hasClass('selected')) {
            selected = ' selected="selected"';
        }
        mobileNav.append('<option' + selected + ' value="' + item.attr('href') + '">' + item.text() + '</option>');
    });

    if (subNav.length) {
    	if ($('a', subNav).length > 1) {
	        $('a', subNav).each(function() {
	            ...
	            mobileSubNav.append('<option' + selected + ' value="' + item.attr('href') + '">' + item.text() + '</option>');
	        });
	        mobileNav.after(mobileSubNav);
	    }
    }

    nav.after(mobileNav);

    $('#main-nav-mobile, #sub-nav-mobile').on('change', function() {
